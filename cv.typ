  ...
  #block(width: 100%, breakable: true)[
    *#link("https://arculus.de")[arculus GmbH]* #h(1fr) *Munich, Germany*
    ...
    #block(width: 100%, breakable: true, above: 0.6em)[
      ...
      Working on a novel master control system for fleets of autonomous mobile robots, fully written in *#emph("Rust")*. I built the first prototype and developed it with my team into a robust production system running at customers.

      - Developed graph-based algorithms for preventing deadlocks between vehicles in large fleets *#text(style: "italic")[(Rust)]*
      - Designed and implemented our scalable Kubernets-based simulation system capable of simulating hundreds of fleets at the same time *#text(style: "italic")[(Rust)]*
      - Implemented a deterministic re-simulation approach that allows us investigate issues reported by customers much quicker than using conventional methods, presented at *#emph(text(blue)[#link("https://youtu.be/ao-CLgci-e8?si=C2SUibtwsYeThRAl")[Oxidize Conf 2024]])*
      - Developed our internal integration test framework that allows setting up and testing complex scenarios using simulated robots using just high-level directives, providing a cornerstone of our testing-strategy *#emph("(Rust)")*
      - Developed an obstacle avoidance module for our mobile robots *#text(style: "italic")[(C++)]*
      - Designed and built a platform for collecting diagnostic data from deployed robot fleets, helping first-level support to identify common issues without requiring support from domain-experts *#text(style: "italic")[(Rust, MongoDB, MinIO)]*
    ]
  ]
  ...
  #block(width: 100%, breakable: true)[
    *Airbus Defence and Space* #h(1fr) *Manching, Germany*
    ...
    #block(width: 100%, breakable: true, above: 0.6em)[
      ...
      - Built a flightpath-planning algorithm for reconnaissance of enemy air defense that allows using multiple UAVs to improve speed and accuracy *#emph("(Ada)")* that was successfully flight tested
      - Built tooling for re-simulation of the mission computer that decreased time for reproducing bugs observed during flight tests and HIL testing *#emph("(C++)")*
...
#block[
  ...
  #block(width: 100%, breakable: true)[
    ...
    *University College London* #h(1fr) *London, United Kingdom* \
    ...
    - *Master Project:* Implementation and Simulation-based Evaluation of Visual-Inertial Marker Integration for an Optical Motion Capture System
  ]
...
#block[
  ...
  - *Other Technologies (non-exhaustive List)*
